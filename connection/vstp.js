...
class Subscription {
    constructor(_client){
        ...
        const headers = {
            destination: `/topic/VSTP_ALL`, // subscribe for a destination to which messages are sent
        ...
        this._subscription = this._client.subscribe(headers, (err, message) => {
            ...
            message.readString('utf-8', (err, body) => {
                ...
                if(body){
                    ...
                    if(data){
                        var data = data.VSTPCIFMsgV1
                        var speed = parseInt(data.schedule.schedule_segment.CIF_speed)
                        if(speed !== "") data.schedule.schedule_segment.CIF_speed = Math.round(speed / 2.24)
                        else data.schedule.schedule_segment.CIF_speed = 0
